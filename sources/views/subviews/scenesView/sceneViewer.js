...
export default class ScenesViewerWithControls extends JetView {
	...
	_attachControlsEvents() {
		...
		this.on(rotationSlider, "onChange", (val) => {
			this._changeRotation(val);
		});
		...
		this.on(colorizeSlider, "onChange", (val) => {
			this._changeColorize(val);
		});
		...
		this.on(hueSlider, "onChange", (val) => {
			this._changeHue(val);
		});
		...
		this.on(brightnessSlider, "onChange", (val) => {
			this._changeBrightness(val);
		});
		...
		this.on(contrastSlider, "onChange", (val) => {
			this._changeContrast(val);
		});
	}
	...
	async onImageSelect(image) {
		...
		if (osdViewerTemplate.isVisible()) {
			...
			this._layer.tileSources = this._layer.tileSources ||
			...
			this._layers[image._id] = this._layer;
		}
		else {
	...
	setLayers(images) {
		this._layers = images.reduce((acc, image) => {
			acc[image._id] = MakerLayer.makeLayer({name: image.name});
			return acc;
		}, {});
	}
}
